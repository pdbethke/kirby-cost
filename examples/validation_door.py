"""Runnable walkthrough of ``kirby_cost.validation`` -- the four doors a
...
ONE EXCEPTION, at the end: ``verify()``'s framework half needs a Multipower
with slots, and ``build_power_from_spec`` builds standalone powers only (it
has no slot wiring). That last section is a DEMONSTRATION on the project's
own test character, generated by ``tests/validation_sink.py`` -- so it runs
only from a source checkout, and it is not a shape consuming code should
copy. Everything above it is.
...
def main() -> None:
    ...
    # (`blast` was rebound above to the Blast now carrying HALFRANGEMODIFIER --
    # a legal modifier, so this walk has nothing to say about it.)
    print(f"\nverify(the Blast carrying Half Range Modifier) -> {verify(blast)}")
